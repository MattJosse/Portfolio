...
def naiveSoftmaxLossAndGradient(
    ...
    Arguments:
    centerWordVec -- numpy ndarray, center words embedding
    ...
    smax = softmax(np.array([np.dot(w,centerWordVec) for w in outsideVectors]))

    loss=-np.log(smax[outsideWordIdx])

    yy=smax
    yy[outsideWordIdx]-=1

    gradCenterVec= outsideVectors.T @ (yy) 
    
    gradOutsideVecs=np.zeros_like(outsideVectors)
    for i in range(len(outsideVectors)):
        gradOutsideVecs[i]=smax[i]*centerWordVec
...
def negSamplingLossAndGradient(
    ...
    sigmoid_neg_score_ks = np.reshape(sigmoid(-scores[1:]), (-1, 1)) # (K, 1)
    gradOutsideVecs=np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx]=(sigmoid(scores[0])-1)*centerWordVec



    for i, k in enumerate(negSampleWordIndices):
        gradOutsideVecs[k] += (1 - sigmoid_neg_score_ks[i]) * centerWordVec 
...
&                     in shape (num words in vocab, word vector length)
    ...
    outsideWordVectors=outsideVectors[[word2Ind[w] for w in outsideWords]]

    for ow in outsideWords:
        loss_o, gradcenterVec_o ,gradOutsideVectors_o = word2vecLossAndGradient(centerWordVec, word2Ind[ow], outsideVectors, dataset)
        loss+=loss_o
        gradCenterVecs[word2Ind[currentCenterWord]]+=gradcenterVec_o
